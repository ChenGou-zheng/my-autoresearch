...
goals, datasets, metrics, commands, paths, and protected files in `project.md`.
...
Ensure `todo.md` has a clear top task, and `next_run.json` has the next model
...
`opencode run` session from `next_run.json`.
